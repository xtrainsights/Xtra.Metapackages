...
namespace Xtra.Metapackages.Serilog
{
    ...
    public class LookupDestructuringPolicy<TKey, TElement> : IDestructuringPolicy
    {
        ...
        {
            if (value is IEnumerable<IGrouping<TKey, TElement>> lookup) {
    ...
    /// <see cref="T:Xtra.Metapackages.Serilog.LookupDestructuringPolicy" /> is roughly 25-30% slower than the generic
    ...
    public class LookupDestructuringPolicy : IDestructuringPolicy
    {
        public bool TryDestructure(object value, ILogEventPropertyValueFactory propertyValueFactory, out LogEventPropertyValue result)
        {
            ...
            var groupingType = _typeCache.GetOrAdd(objectType, GetGroupingType);
        ...
        private static Type GetGroupingType(Type t)
            => t.GetInterfaces()
                ...
                .Select(i => i.GetGenericArguments().FirstOrDefault(a => a.GetGenericTypeDefinition() == typeof(IGrouping<,>)))
                .FirstOrDefault();


        private readonly ConcurrentDictionary<Type, Type> _typeCache = new();
    }
